...
var DEFAULT_PROFILE_PATH = (0, import_path.normalize)((0, import_path.join)(xdg({ subdir: "ObsidianPlugins" }).data, "Profiles"));
...
var SettingsProfilesSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian4.Setting(containerEl).setHeading().setName("Statusbar interaction").setDesc("Change the behavior when clicked on the Status bar Icon");
    new import_obsidian4.Setting(containerEl).setName("Click").addDropdown(
      (dropdown) => dropdown.addOptions(STATUSBAR_CLICK_ACTIONS).setValue(this.plugin.getStatusbarInteraction()).onChange((value) => {
        const action = value;
        this.plugin.setStatusbarInteraction(action);
        this.display();
      })
    );
    new import_obsidian4.Setting(containerEl).setName("Ctrl + Click").addDropdown(
      (dropdown) => dropdown.addOptions(STATUSBAR_CLICK_ACTIONS).setValue(this.plugin.getStatusbarInteraction("ctrl")).onChange((value) => {
        const action = value;
        this.plugin.setStatusbarInteraction(action, "ctrl");
        this.display();
      })
    );
    new import_obsidian4.Setting(containerEl).setName("Shift + Click").addDropdown(
      (dropdown) => dropdown.addOptions(STATUSBAR_CLICK_ACTIONS).setValue(this.plugin.getStatusbarInteraction("shift")).onChange((value) => {
        const action = value;
        this.plugin.setStatusbarInteraction(action, "shift");
        this.display();
      })
    );
    new import_obsidian4.Setting(containerEl).setName("Alt + Click").addDropdown(
      (dropdown) => dropdown.addOptions(STATUSBAR_CLICK_ACTIONS).setValue(this.plugin.getStatusbarInteraction("alt")).onChange((value) => {
        const action = value;
...
var SettingsProfilesPlugin = class extends PluginExtended {
  ...
  updateUI() {
    ...
    if (this.statusBarItem) {
      ...
      this.statusBarItem = this.addStatusBarItem(icon, profile == null ? void 0 : profile.name, label, (ev) => {
        try {
          const loadCallback = (profile3) => {
            this.loadProfileSettings(profile3).then((profile4) => {
              this.updateCurrentProfile(profile4);
              new DialogModal(this.app, "Reload Obsidian now?", "This is required for changes to take effect.", () => {
                this.saveSettings().then(() => {
                  this.app.commands.executeCommandById("app:reload");
                });
          ...
          const saveCallback = (profile3) => {
            this.saveProfileSettings(profile3).then(() => {
              new import_obsidian7.Notice("Saved profile successfully.");
            });
          };
          ...
          if (modifiers > 1) {
            ...
            if (!profile2 || this.isProfileSaved(profile2)) {
              if (!profile2 || this.isProfileUpToDate(profile2)) {
                ...
                loadCallback(profile2);
              }
            } else {
              saveCallback(profile2);
            }
          ...
          console.error(e);
          new import_obsidian7.Notice("Something went wrong check console for more information!");
        }
      });
    }
  }
   ...
   */
  getProfilesPath() {
    const deviceID = (0, import_node_machine_id.machineIdSync)(false);
    ...
    if (!devicePath || devicePath === "") {
      ...
      if (deprecatedPath && deprecatedPath !== "") {
        ...
        this.saveSettings();
        return (0, import_path4.normalize)(deprecatedPath);
      } else {
        ...
        this.saveSettings();
        return (0, import_path4.normalize)(DEFAULT_PROFILE_PATH);
      }
    }
    return (0, import_path4.normalize)(devicePath);
  }
  /**
  ...
  setProfilePath(path) {
    ...
    path = path.trim();
    if (path !== "") {
      this.vaultSettings.devices[deviceID] = (0, import_path4.normalize)(path);
    } else {
  ...
  setStatusbarInteraction(value, mod) {
    switch (mod) {
      case "ctrl":
